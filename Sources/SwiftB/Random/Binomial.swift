...
public struct BinomialDistribution: DiscreteRandomVariable {
    ...
    public func value() -> Int {
        return generateBinomial(trials: trials, probability: probability)
...
let BINV_CUTOFF = 110
let SMALL_MEAN = 30.0
let FAR_FROM_MEAN = 20

func generateBinomial(trials: Int, probability: Double) -> Int {
    ...
    let r: Int
    if np < SMALL_MEAN {
        r = binomialInverseCDF(p: p, n: trials)
    } else {
        r = binomialBTPE(p: p, n: trials, np: np)
    }
...
func binomialInverseCDF(p: Double, n: Int) -> Int {
    let q: Double = 1 - p
    let s: Double = p / q
...
func binomialBTPE(p: Double, n: Int, np: Double) -> Int {
    let q: Double = 1 - p
    let s: Double = p / q
...
fileprivate func _amaxp(k: Int, npq: Double) -> Double {
...
fileprivate func _stirlingAccept(n: Int, ix: Int, fm: Double, m: Int, p: Double, q: Double, xm: Double) -> Double {
...
fileprivate func _stirling(_ x: Double) -> Double {
    let y: Double = x * x
